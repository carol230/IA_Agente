...
class EntornoGrid:
    ...
    def dibujar(
        ...
        pantalla.blit(
            img_agente, (self.agente_pos[0] * CELL_SIZE, self.agente_pos[1] * CELL_SIZE)
        )
        ...
        controles_setup = font.render(
            "SETUP: Mover con flechas. F=Fruta, V=Veneno, W=Pared. 'C' para limpiar.",
            True,
            COLOR_TEXTO,
        )
...
def main():
    ...
    try:
        ...
        print("Advertencia: No se encontró 'pared.png'. Se usará un cuadrado verde.")
                ...
                if evento.key == pygame.K_t:
                    if modo_juego != "TRAINING":
                        ...
                        agente = AgenteQLearning(
                            num_estados=(GRID_HEIGHT, GRID_WIDTH), num_acciones=4
                        )
                        pantalla.fill(COLOR_FONDO)
                        font = pygame.font.Font(None, 50)
                        texto_entrenando = font.render(
                            "Entrenando...", True, COLOR_TEXTO
                        )
                        rect = texto_entrenando.get_rect(
                        ...
                        for episodio in range(NUM_EPISODIOS_ENTRENAMIENTO):
                            ...
                            while not terminado:
                                ...
                                agente.actualizar_q_table(
                                    estado, accion, recompensa, nuevo_estado
                                )
                                estado = nuevo_estado
                            agente.decaimiento_epsilon()
                            if (episodio + 1) % 1000 == 0:
                                print(
                                    f"Episodio: {episodio + 1}/{NUM_EPISODIOS_ENTRENAMIENTO}, Epsilon: {agente.epsilon:.4f}"
                                )
