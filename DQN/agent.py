...
class Agent:
    def __init__(self, state_shape, action_size):
        ...
        self.memory = deque(maxlen=20000) # Más memoria para experiencias más ricas
        self.gamma = 0.99  # Un gamma más alto para pensar más a largo plazo
        self.epsilon = 1.0
        self.epsilon_min = 0.01
        self.epsilon_decay = 0.9995 # Decaimiento aún más lento
        self.learning_rate = 0.0001 # Tasa de aprendizaje más baja para mayor estabilidad
    ...
    def replay(self, batch_size):
        ...
        # Calcular los Q-values para los siguientes estados usando la red de destino
        with torch.no_grad():
            # Usamos target_model para obtener los valores del siguiente estado
            next_q_values = self.target_model(next_states).max(1)[0].unsqueeze(1)

        # Calcular el valor de destino (target)
        # Si el episodio terminó (done=True), el target es solo la recompensa
        ...
        loss.backward()
        torch.nn.utils.clip_grad_value_(self.model.parameters(), 1) # Ayuda a estabilizar
        self.optimizer.step()

        if self.epsilon > self.epsilon_min:
            self.epsilon *= self.epsilon_decay
            
        # Actualizar la red de destino cada N episodios (mejor hacerlo cada N pasos)
        # Lo haremos dentro del bucle de entrenamiento en train.py
    def load(self, name):
